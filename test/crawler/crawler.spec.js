...
describe('Crawler test', function () {
  it('Test with simple echo bot', async function () {
    ...
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'] })
  ...
  it('Test simple echo bot with depth', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'], depth: 3 })
    const flatConvos = _.flatten(crawlerResult.convos)
  ...
  it('Test simple echo bot with waitForPrompt', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'], waitForPrompt: 100 })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 6)
    assert.equal(flatConvos[0].header.name, 'FRUITS_Convo_1')
    assert.equal(flatConvos[0].conversation.length, 6)
  ...
  it('Test simple echo bot with exitCriteria for text', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'], exitCriteria: ['Gree'] })
    const flatConvos = _.flatten(crawlerResult.convos)
  ...
  it('Test simple echo bot with ignoreButtons for payload', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'], ignoreButtons: ['{"name":"Apple"}'] })
    const flatConvos = _.flatten(crawlerResult.convos)
  ...
  it('Stop conversation at first step', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Hello'] })
    const flatConvos = _.flatten(crawlerResult.convos)
  ...
  it('Welcome message incorrect', async function () {
    process.env.BOTIUM_CONFIG = path.resolve(__dirname, CONFIG_DIR, 'fruits.json')
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
  ...
  it('Welcome message', async function () {
    process.env.BOTIUM_CONFIG = path.resolve(__dirname, CONFIG_DIR, 'fruits_with_welcome_message.json')
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ numberOfWelcomeMessages: 2 })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 1)
    assert.equal(flatConvos[0].header.name, 'WELCOME_MESSAGE_Convo_1')
    assert.equal(flatConvos[0].conversation.length, 2)
  })

  it('Welcome message dynamic', async function () {
    process.env.BOTIUM_CONFIG = path.resolve(__dirname, CONFIG_DIR, 'fruits_with_welcome_message.json')
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ dynamicWelcomeMessages: true })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 1)
    assert.equal(flatConvos[0].header.name, 'WELCOME_MESSAGE_Convo_1')
    assert.equal(flatConvos[0].conversation.length, 2)
  ...
  it('Test simple echo bot with userAnswers', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Hello'], userAnswers })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 1)
    assert.equal(flatConvos[0].header.name, 'HELLO_Convo_1')
    assert.equal(flatConvos[0].conversation.length, 4)
  ...
  it('Test simple echo bot with end of conversation', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Fruits'], endOfConversations })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 4)
    assert.equal(flatConvos[0].header.name, 'FRUITS_Convo_1')
    assert.equal(flatConvos[0].conversation.length, 4)
  ...
  it('Test with simple echo bot with url button', async function () {
    ...
    const driver = new BotDriver()
    const crawler = new Crawler({ driver })
    const crawlerResult = await crawler.crawl({ entryPoints: ['Give me url buttons'] })
    const flatConvos = _.flatten(crawlerResult.convos)
    assert.equal(flatConvos.length, 1)
