...
module.exports = class ConvoHandler {
  ...
  async _getConversationScripts (convo) {
    ...
    return { name: convo.header.name, script: scriptDecompiled, stucked: convo.stucked, path: convo.path, botUtterances: utterances.bot, meUtterances: utterances.me, err: convo.err }
  }
      ...
      if (utterance.occurances.length > 1) {
        for (const scirptObject of replacedScriptObjects) {
          for (const occurance of utterance.occurances) {
            scirptObject.script = scirptObject.script.replace(
